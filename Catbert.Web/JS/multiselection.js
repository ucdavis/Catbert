...
jQuery.fn.mselect = function(options) {
    options = jQuery.extend({
        name: "default", //test options
        size: 5,
        truefalse: true,
        defaultTab: "selected"
    }, options);
    
    jQuery.fcbkListSelection(this.selector, 400, 50, 2, options.defaultTab);
}

jQuery.fcbkListSelection = function(elem, width, height, row, defaultTab) {

    var setTabActive = function(tab) {
        $("li.view_on").removeClass("view_on"); //Remove the viewon class from all selections
        var obj = $("li[id=view_" + tab + "]"); //find the correct tab
        obj.addClass("view_on");
        getContent(elem, tab); //Get the content
    }
